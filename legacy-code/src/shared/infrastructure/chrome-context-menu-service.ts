...
export class ChromeContextMenuService implements BrowserContextMenuService {
  removeFeatureContextMenus(featureName: string, listeners: ContextMenuListener[]): void {
    for (const listener of listeners) {
      chrome.contextMenus.remove(listener.name, () => void chrome.runtime.lastError);
    }
    chrome.contextMenus.remove(featureName, () => void chrome.runtime.lastError);
  }

  createFeatureContextMenus(featureName: string, listeners: ContextMenuListener[]): void {
