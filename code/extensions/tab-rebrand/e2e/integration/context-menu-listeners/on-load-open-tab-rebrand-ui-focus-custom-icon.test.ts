...
/*
 * Functional Test: Open Tab Rebrand UI (Focus Custom Icon) via Context Menu
 */

test.describe('Context Menu - Open Tab Rebrand UI Focus Custom Icon', () => {
  let page: Page;
  ...
  test('should verify context-menu-open-tab-rebrand-ui-focus-custom-icon listener is registered', async ({ context }) => {
    ...
    await expect.poll(async () => {
      return await background.evaluate(async () => {
        return await new Promise<boolean>((resolve) => {
          const menuItemId = 'context-menu-open-tab-rebrand-ui-focus-custom-icon';
