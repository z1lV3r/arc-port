...
export class OnTabActivatedSetIcon implements TabEventListener {
  ...
  constructor(browserExtensionActionService: BrowserExtensionActionService, getCheckpointUseCases: GetCheckpointUseCases) {
  ...
  async command(tabId: string): Promise<void> {
    const checkpoint = await this.getCheckpointUseCases.getCurrentTabCheckpoint();
    if (checkpoint) {
