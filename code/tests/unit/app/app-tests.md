...
**File:** `code/src/app/domain/use-cases/context-menu-listener-use-case.ts`
...
**File:** `code/src/app/domain/use-cases/shortcut-listener-use-case.ts`
...
**File:** `code/src/app/domain/use-cases/tab-event-listener-use-case.ts`

When `registerOnCreateTabEventListeners` is called with listener arrays
- the browser should execute those listeners when a tab is created.

When `registerOnUpdateTabEventListeners` is called with listener arrays
- the browser should execute those listeners when a tab is updated.
...
When `registerOnCreateTabEventListeners` is called with an empty array
- no listeners should be executed when a tab is created.

When `registerOnUpdateTabEventListeners` is called with an empty array
- no listeners should be executed when a tab is updated.

When `registerOnCloseTabEventListeners` is called with an empty array
- no listeners should be executed when a tab is closed.

When all three event listener registration methods are called with different features
- each event type should trigger only its corresponding listeners.
- tab create events should not trigger update or close listeners.
- tab update events should not trigger create or close listeners.
- tab close events should not trigger create or update listeners.
