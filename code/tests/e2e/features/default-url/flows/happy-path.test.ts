...
test.describe('Default URL Happy Path', () => {
  ...
  test('invoke all the operations using MessageEventSender', async ({ context, extensionId }) => {
    ...
    // - Validate there should not be any change in default url
    defaultUrlRes = await messageService.sendGetCurrentTabDefaultUrlEventMessage();
    expect(defaultUrlRes).toBe('https://example.com/');

    // 5. Reset or close tab to default url
    const finalPagePromise = context.waitForEvent('page', {
      predicate: (p) => !p.url().startsWith('chrome-extension://') && !p.url().startsWith('about:'),
    });
    await messageService.sendResetOrCloseCurrentTabToDefaultUrlEventMessage();
    ...
    // 6. Clear default url
    await messageService.sendClearCurrentTabDefaultUrlEventMessage();
    
    // - Validate there should not be any default url
    defaultUrlRes = await messageService.sendGetCurrentTabDefaultUrlEventMessage();
