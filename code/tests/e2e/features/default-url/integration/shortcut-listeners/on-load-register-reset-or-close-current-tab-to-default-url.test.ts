...
test.describe('Shortcut - Reset or close current tab to default URL', () => {
  let page: Page;
  ...
  test('should register the "Reset or close current tab to default URL" shortcut', async ({ context }) => {
    ...
    const command = commands.find(c => c.name === 'shortcut-reset-or-close-current-tab-to-default-url');
    
    expect(command, 'Command should be registered').toBeDefined();
    expect(command?.description).toBe('Reset or close current tab to default URL');
