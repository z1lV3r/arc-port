...
import type { SetTabCheckpointIfUnsetMessageEventListener } from "./set-checkpoint-use-cases-listeners/set-tab-checkpoint-if-unset-message-event-listener";
...
export class SetCheckpointMessageEventSender {
  ...
  constructor(
    ...
    listeners: [SetCurrentTabCheckpointMessageEventListener, SetTabCheckpointIfUnsetMessageEventListener],
  ) {
    this.browserMessageService = browserMessageService;
    [this.setCurrentTabCheckpointMessageEventListener, this.setTabCheckpointIfUnsetMessageEventListener] = listeners;
  }

  async sendSetCurrentTabCheckpointEventMessage(): Promise<string> {
    const response = await this.browserMessageService.sendEventMessage(this.setCurrentTabCheckpointMessageEventListener.name, {});
    return response?.data ?? "";
  }

  async sendSetTabCheckpointIfUnsetEventMessage(tabId: number): Promise<string> {
    const response = await this.browserMessageService.sendEventMessage(this.setTabCheckpointIfUnsetMessageEventListener.name, { tabId });
    return response?.data ?? "";
  }
